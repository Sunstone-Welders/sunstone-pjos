...
export default function StoreModePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (tenant.pricing_mode === 'tier') {
        ...
        // Load full tier data and custom prices for tier-based price resolution
  ...
  // Init store queue mode from tenant
  useEffect(() => {
    if (tenant?.store_queue_mode) setQueueModeOn(true);
  }, [tenant]);
  ...
  const toggleQueueMode = async () => {
    ...
    setQueueModeOn(next);
    try {
      await supabase.from('tenants').update({ store_queue_mode: next }).eq('id', tenant!.id);
    } catch {
    ...
    <div className="fixed inset-0 bg-[var(--surface-base)] flex flex-col">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className={`p-2 rounded-lg min-h-[44px] min-w-[44px] flex items-center justify-center transition-colors ${
              queueModeOn
                ? 'bg-[var(--accent-50)] text-[var(--accent-600)] border border-[var(--accent-500)]'
                : 'hover:bg-[var(--surface-raised)] text-[var(--text-tertiary)] border border-transparent'
            }`}
      ...
      {showFullScreenQR && tenant && (
        <FullScreenQR
          ...
          tenantName={tenant.name}
          {...(queueModeOn
            ? { tenantId: tenant.id, mode: 'store' as const }
